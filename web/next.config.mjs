...
const nextConfig = {
  // Raise the middleware request-body cap (default 10MB) so large multipart
  // uploads survive the middleware layer. Media uploads (e.g. LinkedIn template
  // videos) are capped at 25MB by the backend; 30MB leaves headroom for
  // multipart boundary overhead. Without this, bodies >10MB are truncated and
  // the upload proxy's `req.formData()` throws → "Internal error".
