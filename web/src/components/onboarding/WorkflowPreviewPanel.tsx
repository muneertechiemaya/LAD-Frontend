    ...
    <>
      ...
      >
        ...
        <Controls
          ...
          style={{
            background: 'var(--controls-bg, rgba(255,255,255,0.95))',
...
export default function WorkflowPreviewPanel({
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--panel-bg, #fff)', borderLeft: '1px solid var(--border-color, #f0f0f0)'}} className="[--panel-bg:#ffffff] [--canvas-bg:#fafafa] [--border-color:#f0f0f0] [--text-main:#111827] [--text-muted:#9ca3af] [--dot-color:#e5e7eb] [--controls-bg:rgba(255,255,255,0.95)] [--btn-bg:#f3f4f6] [--btn-text:#111827] [--popover-bg:#ffffff] dark:[--panel-bg:#0b0f19] dark:[--canvas-bg:#0d121f] dark:[--border-color:#1e293b] dark:[--text-main:#f8fafc] dark:[--text-muted:#64748b] dark:[--dot-color:#334155] dark:[--controls-bg:rgba(15,23,42,0.95)] dark:[--btn-bg:#1e293b] dark:[--btn-text:#f8fafc] dark:[--popover-bg:#151f32]">
      {/* ─── HEADER ─── */}
      <div style={{ flexShrink: 0, padding: '14px 18px', borderBottom: '1px solid var(--border-color, #f0f0f0)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-main, #111827)', letterSpacing: '-0.02em' }}>
                Workflow Builder
              </div>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{
                ...
                borderRadius: 16, background: 'var(--canvas-bg, #f9fafb)',
                border: '2px dashed var(--border-color, #d1d5db)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                <Workflow style={{ width: 24, height: 24, color: 'var(--text-muted, #9ca3af)' }} />
              </div>
              <div style={{ color: 'var(--text-muted, #9ca3af)', fontSize: 13, fontWeight: 500 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{
                ...
                background: pickerState !== 'closed' ? 'var(--text-main, #111827)' : 'var(--btn-bg, #f3f4f6)',
                color: pickerState !== 'closed' ? 'var(--panel-bg, #fff)' : 'var(--btn-text, #111827)',
            ...
            {pickerState !== 'closed' && (
              <div style={{
                ...
                width: 280, background: 'var(--popover-bg, #fff)', borderRadius: 16,
                border: '1px solid var(--border-color, #e5e7eb)',
                boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.1)',
                padding: 12, overflow: 'hidden',
                animation: 'mk-slide-up 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
              }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-muted, #9ca3af)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8, paddingLeft: 4 }}>
                  ...
                  {!selectedPlatform ? (
                    PLATFORMS.map((p) => (
                      <button
                        ...
                        onMouseEnter={(e) => e.currentTarget.style.background = 'var(--canvas-bg, #f9fafb)'}
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-main, #111827)' }}>{p.label}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-muted, #9ca3af)' }}>{p.desc}</div>
                        </div>
                    ...
                    <>
                      {PLATFORM_ACTIONS[selectedPlatform.id].map((act) => (
                        ...
                        >
                          ...
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-main, #111827)' }}>{act.title}</div>
                            <div style={{ fontSize: 11, color: 'var(--text-muted, #9ca3af)' }}>{selectedPlatform.label}</div>
                          </div>
                        </button>
                      ))}
                      <button
                        ...
                        style={{
                          ...
                          background: 'var(--canvas-bg, #f9fafb)', borderRadius: 8, color: 'var(--text-muted, #6b7280)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ height: 16, width: 1, background: 'var(--border-color, #e5e7eb)' }} />
            <div style={{ fontSize: 11, color: 'var(--text-muted, #9ca3af)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
              Quick Actions:
              <span style={{ color: 'var(--text-main, #111827)', cursor: 'help' }}>Auto-Optimize</span>
              <span style={{ color: 'var(--text-main, #111827)', cursor: 'help' }}>Smart Delay</span>
            </div>
          </div>
        </div>
      </div>
