...
export function CallLogModal({
  ...
  const signedRecordingUrl = useMemo(() => {
    if (recordingUrlQuery.data?.signed_url) return recordingUrlQuery.data.signed_url;
    if (recordingUrlQuery.data?.data?.signed_url) return recordingUrlQuery.data.data.signed_url;
    if (log?.signed_recording_url) return log.signed_recording_url;
    if (log?.recording_url) return log.recording_url;
    if (log?.call_recording_url) return log.call_recording_url;
    return undefined;
  ...
  const analysisExists = analysis && typeof analysis === "object" && Object.keys(analysis).length > 0;
