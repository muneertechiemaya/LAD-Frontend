    ...
    <div className="w-full h-screen overflow-auto bg-slate-50 dark:bg-[#000724]">
      ...
      <Dialog open={followupDialogOpen} onOpenChange={setFollowupDialogOpen}>
        <DialogContent className="max-w-2xl w-full">
          ...
          {/* Channel Tabs */}
          <div className="flex gap-2 border-b border-slate-200 dark:border-blue-950/40 pb-3">
            {([
              ...
              { key: 'whatsapp', label: 'WhatsApp', icon: <MessageCircle className="w-4 h-4" />, color: 'bg-green-600 dark:bg-emerald-600 text-white' },
            ] as const).map(ch => (
              <button
                key={ch.key}
                onClick={() => {
                ...
                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-semibold transition-all ${
                  followupChannel === ch.key ? ch.color : 'bg-slate-100 dark:bg-[#071131] text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800/80 border border-transparent dark:border-blue-950/40'
                }`}
              >
                {ch.icon}
                {ch.label}
            ...
            <div className="space-y-3">
              <div>
                ...
                {connectedSenders.length > 0 ? (
                  <select
                    ...
                    className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-blue-950/40 rounded-md bg-white dark:bg-[#071131] text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#0b1957] dark:focus:ring-sky-500"
                  >
                    {connectedSenders.map(s => (
                      <option key={s.email} value={s.email} className="dark:bg-[#071131]">{s.label}</option>
                    ))}
                  </select>
                ) : (
              ...
              <div>
                ...
                <Input
                  ...
                  className="text-sm dark:bg-[#071131] dark:border-blue-950/40 dark:text-white"
                />
              </div>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs text-[#0b1957] dark:text-sky-400 hover:underline disabled:opacity-50 font-medium"
              >
                {followupPreviewing ? (
            ...
            {followupPreviewing ? (
              <div className="flex flex-col items-center justify-center gap-3 py-10 bg-slate-50 dark:bg-[#071131] rounded-xl border border-slate-200 dark:border-blue-950/40">
              ...
              <Textarea
                ...
                className="resize-none text-sm leading-relaxed dark:bg-[#071131] dark:border-blue-950/40 dark:text-white"
              />
            )}
          ...
          {followupContext && followupContext.pastMessageCount > 0 && (
            <div className="border border-slate-200 dark:border-blue-950/40 rounded-xl overflow-hidden">
              <button
                onClick={() => setFollowupHistoryOpen(v => !v)}
                className="w-full flex items-center justify-between px-4 py-2.5 bg-slate-50 dark:bg-[#071131] hover:bg-slate-100 dark:hover:bg-slate-800/60 text-sm font-medium text-slate-600 dark:text-slate-300 transition-colors"
              >
              ...
              {followupHistoryOpen && (
                <div className="divide-y divide-slate-100 dark:divide-blue-950/40 max-h-52 overflow-y-auto bg-white dark:bg-[#071131]">
